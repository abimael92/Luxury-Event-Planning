    ...
    return (
        ...
        >
            ...
            <motion.div variants={itemVariants} className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl sm:text-3xl font-heading font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
                                {t('vendorDashboard.welcome')} {vendorName}
                            </h1>
                            <Badge variant="secondary" className="bg-green-100 text-green-700 border-green-200 hidden sm:inline-flex">
                                <TrendingUp className="h-3 w-3 mr-1" />
                                +{stats.monthlyGrowth}% {t('vendorDashboard.thisMonth')}
                            </Badge>
                        </div>
                        <Badge variant="secondary" className="bg-green-100 text-green-700 border-green-200 sm:hidden w-fit">
                            <TrendingUp className="h-3 w-3 mr-1" />
                            +{stats.monthlyGrowth}% {t('vendorDashboard.thisMonth')}
                        </Badge>
                    </div>
                ...
                <div className="flex items-center gap-2 lg:gap-3 mt-4 lg:mt-0">
                    <Button variant="outline" size="sm" className="gap-2 hover:bg-white/80 text-xs lg:text-sm">
                        <Download className="h-3 w-3 lg:h-4 lg:w-4" />
                        {t('vendorDashboard.actions.exportReport')}
                    </Button>
                    <Button size="sm" className="bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 shadow-lg gap-2 text-xs lg:text-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                            ...
                            <AnimatePresence>
                                {recentActivities.map((activity, index) => (
                                    <motion.div
                                        ...
                                        className="flex items-start gap-3 lg:gap-4 p-3 lg:p-4 rounded-xl bg-white/50 border border-white/40 hover:bg-white/80 transition-all duration-200 group"
                                    >
                                        <div className={`p-2 rounded-lg bg-white shadow-sm group-hover:shadow-md transition-shadow ${activity.color} flex-shrink-0`}>
                                            <activity.icon className="h-3 w-3 lg:h-4 lg:w-4" />
                                        </div>
                ...
                <motion.div variants={itemVariants} className="space-y-4 lg:space-y-6">
                    ...
                    <Card className="bg-white/60 backdrop-blur-sm border-white/40 shadow-xl hover-lift">
                        ...
                        <CardContent className="space-y-3 lg:space-y-4">
                            {upcomingEvents.map((event, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg bg-gradient-to-br from-purple-500 to-blue-600 flex items-center justify-center text-white text-xs lg:text-sm font-bold shadow-sm flex-shrink-0">
                                            {event.date.split(' ')[1]}
                                        </div>
                            ...
                            <Button variant="ghost" size="sm" className="w-full gap-2 text-muted-foreground hover:text-foreground text-sm">
                    ...
                    <Card className="bg-gradient-to-br from-purple-50 to-blue-50 border-purple-200 shadow-xl">
                        <CardHeader className="pb-3 lg:pb-4">
                            <CardTitle className="text-base lg:text-lg">{t('vendorDashboard.sections.quickActions')}</CardTitle>
                            <CardDescription className="text-sm">
                                {t('vendorDashboard.sections.quickActionsDescription')}
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-2 lg:space-y-3">
                            <Button variant="outline" size="sm" className="w-full justify-start gap-2 lg:gap-3 hover:bg-white/80 h-10 lg:h-12 text-sm">
                                <Calendar className="h-3 w-3 lg:h-4 lg:w-4" />
